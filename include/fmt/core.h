...
inline typename std::enable_if<
  is_compile_string<S>::value, string_view>::type
...
/**
  ...
  ``StringType<char_type>`` and returns a ``fmt::basic_string_view<char_type>``.
  The conversion function must live in the very same namespace as
  ``StringType<char_type>`` to be picked up by ADL. Non-templated string types
  like f.e. QString must return a ``basic_string_view`` with a fixed matching
  char type.

  **Example**::

    namespace ofStringType {

    template <typename Char>
    inline basic_string_view<char_type> to_string_view(const StringType<Char> &S) {
        return { S.data(), S.length() };
    }

    or

    inline basic_string_view<wchar_t> to_string_view(const WideStringType &S) {
        return { S.data(), S.length() };
    }
