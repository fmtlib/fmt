...
class fp {
 ...
 public:
  ...
  FMT_CONSTEXPR fp() : f(0), e(0) {}
  FMT_CONSTEXPR fp(uint64_t f_val, int e_val) : f(f_val), e(e_val) {}
  ...
  FMT_CONSTEXPR boundaries assign_with_boundaries(Double d) {
  ...
  template <typename Double>
  FMT_CONSTEXPR boundaries assign_float_with_boundaries(Double d) {
    assign(d);
...
struct grisu_shortest_handler {
  ...
  constexpr digits::result on_start(uint64_t, uint64_t, uint64_t, int&) const {
    return digits::more;
  }

  // Decrement the generated number approaching value from above.
  FMT_CONSTEXPR void round(uint64_t d, uint64_t divisor, uint64_t& remainder,
                           uint64_t error) {
    while (
  ...
  FMT_CONSTEXPR digits::result on_digit(char digit, uint64_t divisor,
                                        ...
                                        int exp, bool integral) {
    buf[size++] = digit;
    if (remainder >= error) return digits::more;
    uint64_t unit = integral ? 1 : power_of_10_64(-exp);
