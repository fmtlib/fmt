...
bool namespace_detail_invisible() {
    ...
    _MSC_FULL_VER <= 195100000
  // bug in msvc up to at least 18.5:
...
TEST(module_test, vformat_to_n) {
  ...
  auto store = fmt::make_format_args(n);
  std::string s;
//  auto result = fmt::vformat_to_n(std::back_inserter(s), 1, "{}", store); //unused
  char buffer[4] = {0};
...
//  std::initializer_list<int> il{1, 2, 3}; //* unused variable
...
TEST(module_test, printf) {
  ...
  // as_string(L"42.123456")); EXPECT_WRITE(stdout, fmt::printf(L"%d", 42),
  // as_string(L"42"));
}
...
TEST(module_test, vfprintf) {
  ...
  // EXPECT_WRITE(stderr, fmt::vfprintf(stderr, L"%d", wargs),
  // as_string(L"42"));
}
